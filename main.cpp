...
void writeLoginToFile(LoginList *l) {
    ofstream f("login.csv");
    if (!f) {
        cerr << "Error opening login file for writing." << endl;
        return;
    }
    ...
    LoginData *L = l->head;
    while (L != nullptr) {
        f << L->login.id << "," << L->login.password << endl;
        L = L->next;
    }
    f.close();
}
...
void changePassword(LoginList *l) {
    if (!l->head) {
        cout << "No users." << endl;
        return;
    }
...
LoginList *llist = createLoginList();
...
void loginEmployee() {
    ...
    cout << "ID: "; cin >> id;
    cout << "Pass: "; cin >> pass;
    LoginData *L = llist->head;
    while (L) {
        if (L->login.id == id && L->login.password == pass) {
            cout << "Welcome!" << endl;
            employeeBoard();
            return;
        }
